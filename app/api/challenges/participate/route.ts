  ...
  try {
    ...
    if (existingParticipation.length > 0) {
      return NextResponse.json(
        { message: "User already participating in this challenge" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      {
        message: "User participation added successfully",
    ...
    return NextResponse.json(
      { message: "Failed to add participation", error: error.message },
      { status: 500 }
    );
  }
}
