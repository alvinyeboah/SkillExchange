...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ wallet: wallet[0], transactions }, { status: 200 });
  } catch (error: any) {
...
export async function PATCH(req: Request) {
    try {
      ...
      if (!userId || typeof adjustment !== 'number') {
        return NextResponse.json({ message: 'User ID and adjustment are required' }, { status: 400 });
      }
      ...
      return NextResponse.json({ message: 'Failed to update wallet', error: error.message }, { status: 500 });
    }
  }